...
impl AuditLog {
    ...
    pub fn append(
        ...
        let this_hash = compute_hash(
            &prev_hash, &record_id, &alert_id, &tx_hash, &timestamp, &event,
        );
...
mod tests {
    ...
    fn append_only_records_preserve_history() {
        ...
        assert_eq!(
            decisions,
            vec!["under_investigation", "confirmed_suspicious"]
        );
    }
    ...
    fn verify_integrity_detects_tampering() {
...

        {
            ...
            records[0].event = AuditEventKind::Note {
