...
pub fn build_router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route(
            "/api/transactions/:tx_hash/evaluate",
            post(handlers::evaluate_transaction),
        )
        .route("/api/alerts", get(handlers::list_alerts))
        .route("/api/alerts/:alert_id", get(handlers::get_alert))
        .route(
            "/api/alerts/:alert_id/ai-investigation",
            get(handlers::get_ai_investigation),
        )
        .route(
            "/api/alerts/:alert_id/decision",
            post(handlers::submit_decision),
        )
        .route(
            "/api/alerts/:alert_id/audit",
            get(handlers::get_alert_audit),
        )
